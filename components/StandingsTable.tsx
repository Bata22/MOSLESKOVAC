...
export default function StandingsTable({ standings }: { standings: Standing[] }) {
  ...
  //   return String(pos)
  // }
  const medal = (pos: number) => String(pos)
          ...
          <thead>
            <tr>
              {['#','Tim','Utakmice','Pobede','Izgub.','S+','S-','B+','B-','Bodovi'].map((h, i) => (
                <th key={h}
                  className={`px-4 py-3 text-left text-xs font-bold tracking-widest ${i === 0 || i === 9 ? 'text-center' : ''}`}
                  style={{ background: zenski ? 'linear-gradient(135deg, #6b1a3a, #9b2d5a)' : 'rgba(0,45,99,0.8)', color: zenski ? '#f9a8d4' : '#f5c518' }}>
                  {h}
                </th>
              ))}
            </tr>
